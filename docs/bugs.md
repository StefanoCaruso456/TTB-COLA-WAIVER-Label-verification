...
- **Symptom (observed):**
  - `gemini.extract` span: `geminiCallMs: 5700` (5.7s — **0.7s over the 5s SLO threshold, a near-miss not a blowout**), `completionTokens: 1204`, `promptTokens: 1326`, `totalTokens: 2530`, `estimated_cost_usd: $0.0034`.
  - `verify` parent span: `verifyTotalMs: 5782` (5.78s — also just over 5s); end-to-end UX reported ~7.7s (delta = client upload + render).
  - Scores: `extraction.latencyUnder5s: 0` and `verification.latencyUnder5s: 0` — **both SLO scores failed by ~0.7–0.8s.** That proximity matters: a fix that shaves ~1s off the Gemini call (the lite-variant swap proposed below) flips both scores to 1.
